    ...
    <SheetContent class="box-border flex">
      <SheetHeader class="w-full justify-between">
        ...
        <div class="justify-end">
          ...
          <SheetClose asChild>
            <Button
              class="popmake-1867 w-full cursor-pointer"
...
export default {
  ...
  computed: {
    totalProducts() {
      return this.localStorageProducts.reduce(
  ...
  mounted() {
    ...
    allProducts.forEach((product) => {
      ...
      const imageUrl = product
        .querySelector('.wpb_single_image img')
        .getAttribute('src')
    ...
    if (!localStorageProducts) {
      ...
      const newLocalStorageProducts = JSON.parse(localStorageProducts).map(
        (product) => {
          if (!product.quantity) {
            product.quantity = 1
          }
